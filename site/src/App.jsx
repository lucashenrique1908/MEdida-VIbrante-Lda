...
function App() {
  ...
  const employees = useMemo(() => users.filter((u) => u.role === 'empregado'), [users])
  ...
  const addSiteOrder = (payload) => {
    const suggested = nextAvailableDate(orders, payload.date, payload.durationDays)
...
function PublicArea({ route, orders, gallery, reviews, onCreateOrder, onCreateReview, onNavigate }) {
  ...
  const submitOrder = (e) => {
    ...
    const suggestion = nextAvailableDate(orders, form.date, payload.durationDays)
    if (suggestion !== form.date) {
      setError(`Esse servico so vai ser possivel prosseguir a partir de ${suggestion}.`)
      return
    }
    ...
    if (!result.ok) {
      setError(`Esse servico so vai ser possivel prosseguir a partir de ${result.suggested}.`)
      return
    }

    setMessage(`Pedido registado. SMS de confirmacao simulada para ${form.phone}.`)
  }

  const submitReview = (e) => {
    e.preventDefault()
    if (!review.name || !review.text) return
    onCreateReview({ ...review, id: `r-${Date.now()}`, rating: Number(review.rating) })
    ...
    <main className="page">
      {(route === '/' || route === '/sobre') && (
        <section className="card hero">
          <div className="hero-copy">
            ...
            <p>Mais de 20 anos em instalacao, manutencao, limpeza e reparacao residencial e comercial.</p>
            <div className="hero-actions">
              <button onClick={() => onNavigate('/contacto')}>Pedir Orcamento</button>
              <a href={`https://wa.me/${COMPANY.whatsapp}`} target="_blank" rel="noreferrer">Falar no WhatsApp</a>
            </div>
          </div>
          ...
          <div className="grid service-grid">
            {SERVICES.map((service) => (
              <article key={service} className="service-card">
                <h3>{service}</h3>
                <p>Execucao tecnica com foco em performance, conforto e durabilidade.</p>
              </article>
            ))}
          </div>
        </section>
      ...
      {(route === '/' || route === '/projetos') && (
        <section className="card">
          <h2>Projetos</h2>
          <div className="grid">
            {gallery.slice(0, route === '/' ? 6 : 12).map((m) =>
              m.type === 'video' ? <video key={m.id} controls src={m.url} /> : <img key={m.id} src={m.url} alt={m.label} />,
            )}
          </div>
        </section>
      )}
        ...
        <section className="card">
          ...
          <form className="form" onSubmit={submitOrder}>
            {[
              ...
              <label key={field}>
                ...
                <input
                  ...
                  required={['name', 'phone', 'street', 'number', 'postalCode'].includes(field)}
                />
              </label>
            ))}
            ...
            <fieldset className="full chooser">
              ...
              <label>
                <input type="radio" checked={form.channel === 'site'} onChange={() => setForm({ ...form, channel: 'site' })} />
                Seguir com pedido pelo site
              </label>
            </fieldset>

            <button className="full" type="submit">Enviar pedido</button>
          </form>
      ...
      {route === '/' && (
        <section className="card">
          ...
          <form className="form mini" onSubmit={submitReview}>
            <input placeholder="Seu nome" value={review.name} onChange={(e) => setReview({ ...review, name: e.target.value })} />
            <select value={review.rating} onChange={(e) => setReview({ ...review, rating: e.target.value })}>
              <option value={5}>5</option>
              <option value={4}>4</option>
              <option value={3}>3</option>
            </select>
            <textarea placeholder="Escreva seu comentario" value={review.text} onChange={(e) => setReview({ ...review, text: e.target.value })} rows={3} />
...
function PrivateArea({ session, employees, orders, onLogin, onRegister, onLogout, onAddManual, onPatch, onUpload }) {
  const [auth, setAuth] = useState({ username: '', password: '', role: 'empregado' })
      ...
      <main className="page private-page">
        <section className="card auth">
          ...
          >
            ...
            <input placeholder="Utilizador" value={auth.username} onChange={(e) => setAuth({ ...auth, username: e.target.value })} />
            <input type="password" placeholder="Senha" value={auth.password} onChange={(e) => setAuth({ ...auth, password: e.target.value })} />
          ...
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault()
              setMsg(onRegister(auth) ? 'Utilizador criado.' : 'Utilizador ja existe.')
            }}
          >
    ...
    return (
      <main className="page private-page">
        ...
        <section className="card">
          ...
          <div className="grid reviews">
            {orders.filter((o) => o.assignedTo === session.username).map((o) => (
    ...
    <main className="page private-page">
      ...
      <section className="card">
        ...
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault()
            const r = onAddManual({ ...manual, durationDays: Number(manual.durationDays) })
            setMsg(r.ok ? 'Servico criado com sucesso.' : `Sem vaga. Proxima data: ${r.suggested}`)
          }}
        >
          <input placeholder="Nome do cliente" value={manual.customerName} onChange={(e) => setManual({ ...manual, customerName: e.target.value })} />
          <input placeholder="Telefone" value={manual.customerPhone} onChange={(e) => setManual({ ...manual, customerPhone: e.target.value })} />
          <input placeholder="Endereco" value={manual.address} onChange={(e) => setManual({ ...manual, address: e.target.value })} />

          <label>
            Servico
            <select value={manual.serviceType} onChange={(e) => setManual({ ...manual, serviceType: e.target.value })}>
              {SERVICES.map((s) => (
                <option key={s}>{s}</option>
              ))}
            </select>
          ...
          <label>
            ...
            <input
              ...
              onChange={(e) => setManual({ ...manual, durationDays: e.target.value })}
            />
          </label>

          <label>
            Pago?
            <select value={String(manual.paid)} onChange={(e) => setManual({ ...manual, paid: e.target.value === 'true' })}>
          ...
          <label>
            Empregado
            <select value={manual.assignedTo} onChange={(e) => setManual({ ...manual, assignedTo: e.target.value })}>
              <option value="">---</option>
              {employees.map((emp) => (
            ...
            <tbody>
              {orders.map((o) => (
                <tr key={o.id}>
                  ...
                  <td>
                    <select value={String(o.paid)} onChange={(e) => onPatch(o.id, { paid: e.target.value === 'true' })}>
                      <option value="false">Nao</option>
                      <option value="true">Sim</option>
                    </select>
                  </td>
                  <td>
                    <select value={o.assignedTo || ''} onChange={(e) => onPatch(o.id, { assignedTo: e.target.value })}>
                      <option value="">---</option>
                      {employees.map((emp) => (
                  ...
                  <td>
                    <select value={o.status} onChange={(e) => onPatch(o.id, { status: e.target.value })}>
